(function () {
  ...
  function cacheElements() {
    ...
    els.leagueSeasonFilter = document.getElementById("leagueSeasonFilter");
    els.newLeagueMatchButton = document.getElementById("newLeagueMatchButton");
    els.leagueMatchForm = document.getElementById("leagueMatchForm");
    els.leagueMatchFormTitle = document.getElementById("leagueMatchFormTitle");
    els.leagueMatchEditState = document.getElementById("leagueMatchEditState");
    els.leagueMatchSubmitButton = document.getElementById("leagueMatchSubmitButton");
    els.cancelLeagueMatchEditButton = document.getElementById("cancelLeagueMatchEditButton");
    els.leagueHomePlayerSelect = document.getElementById("leagueHomePlayerSelect");
    els.leagueAwayPlayerSelect = document.getElementById("leagueAwayPlayerSelect");
    els.leagueResultPreview = document.getElementById("leagueResultPreview");
    els.leagueStandingsCount = document.getElementById("leagueStandingsCount");
    els.leagueStandingsTable = document.getElementById("leagueStandingsTable");
    els.leagueMatchCount = document.getElementById("leagueMatchCount");
    els.leagueMatchTable = document.getElementById("leagueMatchTable");
    ...
    els.trainingCheckoutPanel = document.getElementById("trainingCheckoutPanel");
    els.trainingCheckoutRest = document.getElementById("trainingCheckoutRest");
    els.trainingCheckoutAttempt = document.getElementById("trainingCheckoutAttempt");
    els.trainingCheckoutTarget = document.getElementById("trainingCheckoutTarget");
    els.trainingCheckoutDarts = document.getElementById("trainingCheckoutDarts");
  ...
  function bindEvents() {
    ...
    els.leagueMatchForm.addEventListener("submit", handleLeagueMatchSubmit);
    els.cancelLeagueMatchEditButton.addEventListener("click", resetLeagueMatchForm);
    els.newLeagueMatchButton.addEventListener("click", () => {
      resetLeagueMatchForm();
      els.leagueMatchForm.querySelector("[name='date']").focus();
    });
    els.leagueSeasonFilter.addEventListener("change", renderLeague);
    ["homePlayerId", "awayPlayerId", "homeLegs", "awayLegs"].forEach((name) => {
      els.leagueMatchForm.elements[name].addEventListener("input", updateLeagueResultPreview);
      els.leagueMatchForm.elements[name].addEventListener("change", updateLeagueResultPreview);
    });
    ...
    els.trainingVisitScore.addEventListener("input", updateTrainingCheckoutPanel);
    els.trainingCheckoutAttempt.addEventListener("change", updateCheckoutDartsState);
    els.trainingBustButton.addEventListener("click", () => {
      const checkoutInfo = readTrainingCheckoutInfo(true);
      if (!checkoutInfo) return;
      handleTrainingVisit(0, checkoutInfo, true);
    });
  ...
  function normalizeData(value) {
    const data = {
      ...
      leagueMatches: Array.isArray(value.leagueMatches) ? value.leagueMatches : [],
    ...
    data.leagueMatches = data.leagueMatches.map((match) => {
      const homeLegs = toNumber(match.homeLegs);
      const awayLegs = toNumber(match.awayLegs);
      return {
        id: match.id || createId("league"),
        date: cleanText(match.date),
        season: cleanText(match.season),
        round: toNumber(match.round),
        homePlayerId: cleanText(match.homePlayerId),
        awayPlayerId: cleanText(match.awayPlayerId),
        homeLegs,
        awayLegs,
        result: getLeagueMatchResult(homeLegs, awayLegs),
        note: cleanText(match.note),
        createdAt: match.createdAt || new Date().toISOString(),
        updatedAt: match.updatedAt || new Date().toISOString()
      };
  ...
  function normalizeTrainingData(value) {
    ...
    return {
      records: records.map((record) => ({
        ...
        checkoutOpportunities: toNumber(record.checkoutOpportunities),
        checkoutAttempts: toNumber(record.checkoutAttempts),
        checkoutHits: toNumber(record.checkoutHits),
        checkoutMisses: toNumber(record.checkoutMisses),
        finishDarts: toNumber(record.finishDarts),
        finishTarget: cleanText(record.finishTarget),
  ...
  function renderSeasonOptions() {
    const seasons = uniqueSorted([
      ...app.data.matches.map((match) => match.season),
      ...app.data.leagueMatches.map((match) => match.season)
    ].filter(Boolean));
    ...
    setSelectOptions(els.leagueSeasonFilter, seasons, "Alle Seasons");
  }
  ...
  function renderLeaguePlayerOptions() {
    const players = getLeaguePlayers();
    [els.leagueHomePlayerSelect, els.leagueAwayPlayerSelect].forEach((select) => {
      const current = select.value;
      select.innerHTML = "<option value=\"\">Spieler waehlen</option>";
      players.forEach((player) => {
        const option = document.createElement("option");
        option.value = player.id;
        option.textContent = player.isPlayer ? `${player.name} (Du)` : player.name;
        select.appendChild(option);
      });
  ...
  function renderLeague() {
    const season = els.leagueSeasonFilter.value;
    const matches = getLeagueMatchesForSeason(season);
    const standings = calculateLeagueStandings(matches);

    els.leagueStandingsCount.textContent = countLabel(standings.length, "Spieler");
    if (!standings.length) {
      els.leagueStandingsTable.innerHTML = renderTableEmpty(11, "Noch keine Liga-Spieler vorhanden.");
    } else {
      els.leagueStandingsTable.innerHTML = standings.map((row, index) => `
        <tr class="${row.isPlayer ? "player-rank-row" : ""} ${getLeagueRankClass(index)}">
          <td><strong>${index + 1}.</strong></td>
          <td>${renderLeaguePlayerName(row, index)}</td>
          <td>${row.games}</td>
          ...
          <td>${renderLeagueRankStatus(index)}</td>
        </tr>
      `).join("");
    }

    els.leagueMatchCount.textContent = countLabel(matches.length, "Spiel");
    if (!matches.length) {
      els.leagueMatchTable.innerHTML = renderTableEmpty(9, "Noch keine Ligaspiele gespeichert.");
      return;
    }

    els.leagueMatchTable.innerHTML = matches
      .slice()
      .sort((a, b) => {
        const roundDiff = (Number(a.round) || 99) - (Number(b.round) || 99);
      ...
      .map((match) => `
        <tr>
          <td>${formatDate(match.date)}</td>
          <td>${match.round ? `${formatNumber(match.round, 0)}.` : "-"}</td>
          <td>${escapeHtml(match.season)}</td>
          <td>${escapeHtml(getLeaguePlayerName(match.homePlayerId))}</td>
          <td><strong>${formatLeagueScore(match)}</strong></td>
          <td>${escapeHtml(getLeaguePlayerName(match.awayPlayerId))}</td>
          <td>${renderLeagueResultBadge(match)}</td>
          <td>${escapeHtml(match.note || "-")}</td>
          <td>
            <div class="row-actions">
              <button class="button small secondary" type="button" data-action="edit-league-match" data-id="${match.id}">Bearbeiten</button>
              <button class="button small danger" type="button" data-action="delete-league-match" data-id="${match.id}">Loeschen</button>
            </div>
          </td>
        </tr>
      `).join("");
  ...
  function renderLeaguePlayerName(row, index) {
  ...
  function renderLeagueRankStatus(index) {
    if (index <= 1) return "<span class=\"league-status-badge direct\">Direktaufstieg</span>";
    if (index <= 4) return "<span class=\"league-status-badge playoff\">Aufstiegsrunde</span>";
    return "<span class=\"league-status-badge neutral\">Liga halten</span>";
  }

  function getLeagueRankClass(index) {
    if (index <= 1) return "league-direct-row";
    if (index <= 4) return "league-playoff-row";
    return "";
  }

  function renderLeagueResultBadge(match) {
    const result = getLeagueMatchResult(match.homeLegs, match.awayLegs);
    if (result === "draw") return "<span class=\"result-badge draw\">Remis</span>";
    if (result === "home") return "<span class=\"result-badge win\">Sieg Spieler 1</span>";
    if (result === "away") return "<span class=\"result-badge win\">Sieg Spieler 2</span>";
    return "<span class=\"result-badge\">Offen</span>";
  }
  ...
  function updateTrainingCheckoutPanel() {
    const game = app.trainingGame;
    const active = Boolean(game && game.status === "running" && isCheckoutPossible(game.playerScore));
    els.trainingCheckoutPanel.classList.toggle("hidden-field", !active);

    if (!active) {
      resetTrainingCheckoutFields();
      return;
    }

    els.trainingCheckoutRest.textContent = `Rest ${game.playerScore}`;
    setSmartCheckoutTarget(game.playerScore);
    updateCheckoutDartsState();
  }

  function setSmartCheckoutTarget(score) {
    if (!score || score % 2 !== 0 || score > 40) return;
    const target = `D${score / 2}`;
    const hasTarget = Array.from(els.trainingCheckoutTarget.options).some((option) => option.value === target);
    if (hasTarget && els.trainingCheckoutAttempt.value === "no") {
      els.trainingCheckoutTarget.value = target;
    }
  }

  function resetTrainingCheckoutFields() {
    if (!els.trainingCheckoutAttempt) return;
    els.trainingCheckoutAttempt.value = "no";
    els.trainingCheckoutDarts.value = "";
    updateCheckoutDartsState();
  }

  function updateCheckoutDartsState() {
    const isHit = els.trainingCheckoutAttempt?.value === "hit";
    els.trainingCheckoutDarts.disabled = !isHit;
    if (!isHit) els.trainingCheckoutDarts.value = "";
  }

  function readTrainingCheckoutInfo(forcedBust) {
    const game = app.trainingGame;
    const inCheckoutRange = Boolean(game && game.status === "running" && isCheckoutPossible(game.playerScore));
    if (!inCheckoutRange) {
      return { inCheckoutRange: false, result: "no", aimedDouble: false, target: "", darts: null };
    }

    const result = els.trainingCheckoutAttempt.value || "no";
    const target = cleanText(els.trainingCheckoutTarget.value);
    const darts = toNumber(els.trainingCheckoutDarts.value);
    const aimedDouble = result === "miss" || result === "hit";

    if (result === "hit") {
      const score = toNumber(els.trainingVisitScore.value);
      if (forcedBust) {
        alert("Wenn du Bust gedrueckt hast, kann der Checkout nicht als getroffen gespeichert werden.");
        return null;
      }
      if (score !== game.playerScore) {
        alert("Wenn du gecheckt hast, muessen die Punkte genau deinem aktuellen Rest entsprechen.");
        return null;
      }
      if (!darts || darts < 1 || darts > 3) {
        alert("Bitte gib ein, mit wie vielen Darts du gecheckt hast: 1, 2 oder 3.");
        return null;
      }
    }

    return { inCheckoutRange, result, aimedDouble, target, darts: result === "hit" ? darts : null };
  }

  function registerCheckoutAttempt(game, checkoutInfo) {
    if (!checkoutInfo?.inCheckoutRange) return;
    game.checkoutOpportunities = (game.checkoutOpportunities || 0) + 1;
    if (!checkoutInfo.aimedDouble) return;
    game.checkoutAttempts = (game.checkoutAttempts || 0) + 1;
    if (checkoutInfo.result === "hit") {
      game.checkoutHits = (game.checkoutHits || 0) + 1;
    } else {
      game.checkoutMisses = (game.checkoutMisses || 0) + 1;
    }
  }

  function appendCheckoutSummary(summary, checkoutInfo) {
    if (!checkoutInfo?.aimedDouble) return summary;
    if (checkoutInfo.result === "hit") {
      return `${summary} - ${checkoutInfo.target} getroffen mit ${checkoutInfo.darts} Dart${checkoutInfo.darts === 1 ? "" : "s"}`;
    }
    return `${summary} - ${checkoutInfo.target} verpasst`;
  }
  ...
  function renderTrainingStats() {
    ...
    const checkoutAttempts501 = sum(records501, "checkoutAttempts");
    const checkoutHits501 = sum(records501, "checkoutHits");
    const checkoutRate501 = checkoutAttempts501 ? (checkoutHits501 / checkoutAttempts501) * 100 : null;
    const averageFinishDarts = average(records501.filter((record) => record.result === "win").map((record) => record.finishDarts));
    ...
    const cards = [
      ...
      ["501 Checkout", formatPercent(checkoutRate501), checkoutAttempts501 ? `${checkoutHits501}/${checkoutAttempts501} Doppel` : "Noch keine Versuche"],
      ["Check-Darts", formatNumber(averageFinishDarts, 1), "bei gewonnenen Legs"],
  ...
  function handleTrainingVisitSubmit(event) {
    event.preventDefault();
    const score = toNumber(els.trainingVisitScore.value);
    const checkoutInfo = readTrainingCheckoutInfo(false);
    if (!checkoutInfo) return;
    handleTrainingVisit(score, checkoutInfo, false);
  }

  function handleTrainingVisit(score, checkoutInfo, forcedBust) {
    const game = app.trainingGame;
    ...
    const playerResult = applyPlayerVisit(game, forcedBust ? 0 : score, checkoutInfo, forcedBust);
  ...
  function applyPlayerVisitLegacy(game, score, checkoutInfo, forcedBust) {
    game.playerVisits += 1;
    registerCheckoutAttempt(game, checkoutInfo);

    const checkoutHit = checkoutInfo?.result === "hit";
    if (forcedBust || score > game.playerScore || game.playerScore - score === 1 || (score === game.playerScore && !checkoutHit)) {
      return {
        finished: false,
  ...
  function applyPlayerVisit(game, score, checkoutInfo, forcedBust) {
    game.playerVisits += 1;
    registerCheckoutAttempt(game, checkoutInfo);
    ...
    if (forcedBust || score > game.playerScore || game.playerScore - score === 1 || (score === game.playerScore && !checkoutHit)) {
      return {
        finished: false,
        summary: appendCheckoutSummary(`Bust - Rest ${game.playerScore}`, checkoutInfo)
      };
    }
    ...
    if (game.playerScore === 0) {
      game.finishDarts = checkoutInfo?.darts || null;
      game.finishTarget = checkoutInfo?.target || "";
      return {
        finished: true,
        summary: appendCheckoutSummary(`Checkout ${score} - AVG ${formatNumber(calculateGameAverage(game, "player"), 2)}`, checkoutInfo)
      };
    }

    return {
      finished: false,
      summary: appendCheckoutSummary(`${score} Punkte - Rest ${game.playerScore}`, checkoutInfo)
    };
  }
  ...
  function finishTrainingGame(result) {
    ...
    const record = {
      ...
      checkoutOpportunities: game.checkoutOpportunities || 0,
      checkoutAttempts: game.checkoutAttempts || 0,
      checkoutHits: game.checkoutHits || 0,
      checkoutMisses: game.checkoutMisses || 0,
      checkoutRate: game.checkoutAttempts ? (game.checkoutHits / game.checkoutAttempts) * 100 : null,
      finishDarts: game.finishDarts || null,
      finishTarget: game.finishTarget || ""
    };
  ...
  function buildTrainingFeedback(record) {
    ...
    const checkoutText = buildCheckoutFeedback(record);

    if (!relevantOpponents.length) {
      if (avg >= 60 && record.result === "win") return `Starkes Leg: ${resultText} mit ${formatNumber(avg, 2)} AVG. Damit spielst du auf Top-Gegner-Niveau.${checkoutText}`;
      if (avg >= 45) return `Solides Leg: ${formatNumber(avg, 2)} AVG. Damit kannst du in vielen Liga-Spielen mithalten.${checkoutText}`;
      return `Du musst weiter ueben, um konstant Druck auf die Top-Platzierungen zu machen. Dein 501 AVG: ${formatNumber(avg, 2)}.${checkoutText}`;
    }
    ...
    if (avg >= (strongest.average || 0) && record.result === "win") {
      return `Mit diesem Spiel schaffst du sogar den staerksten Gegner ${strongest.name} aus deiner Gegnerliste. ${formatNumber(avg, 2)} AVG und Sieg gegen den Computer.${cupText}${checkoutText}`;
    }

    if (reachable) {
      return `Mit diesem Spiel kannst du es mit ${reachable.name} aus deiner Gegnerliste aufnehmen. Dein Trainings-AVG: ${formatNumber(avg, 2)}, Gegner-AVG: ${formatNumber(reachable.average, 2)}.${cupText}${checkoutText}`;
    }

    return `Du musst weiter ueben, um Platz 1 anzugreifen. Zielmarke: ${strongest.name} mit AVG ${formatNumber(strongest.average, 2)}. Dein Leg lag bei ${formatNumber(avg, 2)}.${cupText}${checkoutText}`;
  }

  function buildCheckoutFeedback(record) {
    if (!record.checkoutAttempts) return " Kein Doppel im Checkout aktiv anvisiert.";
    if (record.result === "win" && record.finishTarget) {
      return ` Checkout: ${record.finishTarget} mit ${formatNumber(record.finishDarts, 0)} Dart${record.finishDarts === 1 ? "" : "s"} getroffen.`;
    }
    if ((record.checkoutRate || 0) < 25) {
      return ` Checkout: ${record.checkoutHits}/${record.checkoutAttempts} Doppel getroffen. Hier liegt der naechste Trainingshebel.`;
    }
    return ` Checkout: ${record.checkoutHits}/${record.checkoutAttempts} Doppel getroffen. Das Finish war solide.`;
  }
  ...
  function getTrainingRecordMetric(record) {
    if (record.type === "501") {
      const checkout = record.checkoutAttempts ? ` - Check ${formatPercent(record.checkoutRate)}` : "";
      return `AVG ${formatNumber(record.playerAverage, 2)}${checkout}`;
    }
  ...
  function deleteOpponent(id) {
    ...
    const relatedLeagueMatches = app.data.leagueMatches.filter((match) => match.homePlayerId === id || match.awayPlayerId === id).length;
    const totalRelated = relatedMatches + relatedLeagueMatches;
    const message = totalRelated
      ? `Gegner "${opponent.name}" und ${totalRelated} zugehoerige Spiele loeschen?`
    ...
    app.data.leagueMatches = app.data.leagueMatches.filter((match) => match.homePlayerId !== id && match.awayPlayerId !== id);
  ...
  function handleLeagueMatchSubmit(event) {
    event.preventDefault();
    const formData = readForm(els.leagueMatchForm, leagueMatchFieldTypes);
    ...
    if (!formData.homePlayerId || !formData.awayPlayerId) {
      alert("Bitte beide Spieler auswaehlen.");
      return;
    }
    ...
    if (formData.round !== null && (formData.round < 1 || formData.round > 13)) {
      alert("Der Spieltag muss zwischen 1 und 13 liegen.");
      return;
    }

    if (!validateLeagueLegs(formData.homeLegs, formData.awayLegs)) {
      alert("Best of 6: Die Legs duerfen zusammen maximal 6 ergeben und muessen mindestens ein Leg enthalten.");
      return;
    }

    formData.result = getLeagueMatchResult(formData.homeLegs, formData.awayLegs);
    const now = new Date().toISOString();

    if (app.editingLeagueMatchId) {
      const index = app.data.leagueMatches.findIndex((match) => match.id === app.editingLeagueMatchId);
      if (index !== -1) {
        app.data.leagueMatches[index] = {
          ...app.data.leagueMatches[index],
          ...formData,
          updatedAt: now
        };
  ...
  function editLeagueMatch(id) {
    const match = app.data.leagueMatches.find((item) => item.id === id);
    if (!match) return;
    app.editingLeagueMatchId = id;
    fillForm(els.leagueMatchForm, match);
    els.leagueMatchFormTitle.textContent = "Ligaspiel bearbeiten";
    els.leagueMatchEditState.textContent = "Bearbeitung";
    els.leagueMatchSubmitButton.textContent = "Aenderungen speichern";
    updateLeagueResultPreview();
    showSection("league");
    els.leagueMatchForm.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  function resetLeagueMatchForm() {
    app.editingLeagueMatchId = null;
    els.leagueMatchForm.reset();
    setDefaultLeagueMatchDate();
    els.leagueMatchFormTitle.textContent = "Ligaspiel speichern";
    els.leagueMatchEditState.textContent = "Neu";
    els.leagueMatchSubmitButton.textContent = "Ligaspiel speichern";
    updateLeagueResultPreview();
  }

  function deleteLeagueMatch(id) {
    const match = app.data.leagueMatches.find((item) => item.id === id);
    if (!match) return;
    const label = `${getLeaguePlayerName(match.homePlayerId)} ${formatLeagueScore(match)} ${getLeaguePlayerName(match.awayPlayerId)}`;
    if (!confirm(`Ligaspiel "${label}" loeschen?`)) return;
    app.data.leagueMatches = app.data.leagueMatches.filter((item) => item.id !== id);
    if (app.editingLeagueMatchId === id) resetLeagueMatchForm();
    saveData();
    renderAll();
  }
  ...
  function handleActionClick(event) {
    ...
    if (action === "edit-league-match") editLeagueMatch(id);
    if (action === "delete-league-match") deleteLeagueMatch(id);
  ...
  function exportData() {
    const payload = {
      ...
      leagueMatches: app.data.leagueMatches,
  ...
  function importData(event) {
    ...
    reader.onload = () => {
      try {
        ...
        const ok = confirm(`Import starten? Aktuelle Browser-Daten werden ersetzt. Enthalten: ${imported.opponents.length} Gegner, ${imported.matches.length} persoenliche Spiele und ${imported.leagueMatches.length} Ligaspiele.`);
        if (!ok) return;
        app.data = imported;
        app.editingOpponentId = null;
        app.editingMatchId = null;
  ...
  function resetAllData() {
    ...
    if (!second) return;
    app.data = { opponents: [], matches: [], leagueMatches: [], setups: [], events: [], player: normalizePlayerProfile() };
    app.editingOpponentId = null;
    app.editingMatchId = null;
    app.editingLeagueMatchId = null;
    app.editingSetupId = null;
  ...
  function validateLeagueLegs(homeLegs, awayLegs) {
    if (homeLegs === null || awayLegs === null) return false;
    const home = Number(homeLegs) || 0;
    const away = Number(awayLegs) || 0;
    return home >= 0 && away >= 0 && home + away > 0 && home + away <= 6;
  }

  function getLeagueMatchResult(homeLegs, awayLegs) {
    if (homeLegs === null && awayLegs === null) return "open";
    const home = Number(homeLegs) || 0;
    const away = Number(awayLegs) || 0;
    if (home > away) return "home";
    if (home < away) return "away";
    if (home === away && home + away > 0) return "draw";
    return "open";
  }

  function updateLeagueResultPreview() {
    const homeId = els.leagueMatchForm.elements.homePlayerId.value;
    const awayId = els.leagueMatchForm.elements.awayPlayerId.value;
    const homeLegs = toNumber(els.leagueMatchForm.elements.homeLegs.value);
    const awayLegs = toNumber(els.leagueMatchForm.elements.awayLegs.value);
    const homeName = getLeaguePlayerName(homeId) || "Spieler 1";
    const awayName = getLeaguePlayerName(awayId) || "Spieler 2";

    if (!validateLeagueLegs(homeLegs, awayLegs)) {
      els.leagueResultPreview.textContent = "Ergebnis: noch nicht berechnet";
      return;
    }

    const result = getLeagueMatchResult(homeLegs, awayLegs);
    if (result === "draw") {
      els.leagueResultPreview.textContent = `Ergebnis: ${homeName} ${homeLegs}:${awayLegs} ${awayName} - Unentschieden`;
    } else if (result === "home") {
      els.leagueResultPreview.textContent = `Ergebnis: ${homeName} gewinnt ${homeLegs}:${awayLegs}`;
    } else if (result === "away") {
      els.leagueResultPreview.textContent = `Ergebnis: ${awayName} gewinnt ${awayLegs}:${homeLegs}`;
    } else {
      els.leagueResultPreview.textContent = "Ergebnis: noch nicht berechnet";
    }
  }
  ...
  function getLeaguePlayers() {
    ...
    app.data.opponents
      .filter((opponent) => opponentIncludesCategory(opponent, "liga"))
    ...
    app.data.leagueMatches.forEach((match) => {
      [match.homePlayerId, match.awayPlayerId].forEach((id) => {
        if (!id || players.some((playerItem) => playerItem.id === id)) return;
        const opponent = findOpponent(id);
        players.push({
          id,
          name: opponent ? opponent.name : "Geloeschter Spieler",
          average: opponent ? opponent.average : null,
  ...
  function getLeaguePlayerName(id) {
    if (!id) return "";
    const player = getLeaguePlayers().find((item) => item.id === id);
    return player ? player.name : "Geloeschter Spieler";
  }

  function getLeagueMatchesForSeason(season) {
    return app.data.leagueMatches.filter((match) => !season || match.season === season);
  }

  function calculateLeagueStandings(matches) {
    const rows = new Map();

    getLeaguePlayers().forEach((player) => {
      rows.set(player.id, {
        id: player.id,
        name: player.name,
    ...
    matches.forEach((match) => {
      if (!rows.has(match.homePlayerId)) {
        rows.set(match.homePlayerId, createEmptyLeagueRow(match.homePlayerId));
      }
      if (!rows.has(match.awayPlayerId)) {
        rows.set(match.awayPlayerId, createEmptyLeagueRow(match.awayPlayerId));
      }

      const home = rows.get(match.homePlayerId);
      const away = rows.get(match.awayPlayerId);
      const homeLegs = Number(match.homeLegs) || 0;
      const awayLegs = Number(match.awayLegs) || 0;
      const result = getLeagueMatchResult(homeLegs, awayLegs);
    ...
    return [...rows.values()]
      .map((row) => ({
        ...row,
        legDiff: row.legsWon - row.legsLost
      }))
      .sort(compareLeagueRows);
  }

  function createEmptyLeagueRow(id) {
    return {
      id,
      name: getLeaguePlayerName(id) || "Geloeschter Spieler",
      isPlayer: id === PLAYER_ID,
      average: null,
      games: 0,
      points: 0,
  ...
  function compareLeagueRows(a, b) {
    return numericValue(b.points) - numericValue(a.points)
      || numericValue(b.legDiff) - numericValue(a.legDiff)
      || numericValue(b.legsWon) - numericValue(a.legsWon)
      || numericValue(b.wins) - numericValue(a.wins)
      || a.name.localeCompare(b.name, "de");
  }

  function formatLeagueScore(match) {
    return `${formatNumber(match.homeLegs, 0)}:${formatNumber(match.awayLegs, 0)}`;
  }
  ...
  function setDefaultLeagueMatchDate() {
    if (!els.leagueMatchForm) return;
    els.leagueMatchForm.elements.date.value = new Date().toISOString().slice(0, 10);
  }
