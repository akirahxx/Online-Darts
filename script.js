(function () {
  ...
  function loadData() {
    try {
      ...
      if (!raw) return { opponents: [], matches: [], setups: [] };
      const parsed = JSON.parse(raw);
      return normalizeData(parsed);
    } catch (error) {
      console.warn("Daten konnten nicht geladen werden.", error);
      return { opponents: [], matches: [], setups: [] };
    }
  }
  ...
  function renderMyGame() {
    ...
    const setupSummary = getBestSetupSummary();
    const mentalAverage = average(app.data.matches.map((match) => match.nervousDuring));
    const firstLeg = calculateFirstLegStats(app.data.matches);
    const cards = [
    ...
    els.setupTable.innerHTML = app.data.setups.map((setup) => {
      const setupMatches = app.data.matches.filter((match) => match.setupId === setup.id);
  ...
  function renderDashboardStats() {
    const stats = calculateStats(app.data.matches);
    const streak = calculateResultStreak(app.data.matches);
    const cards = [
      ["Spiele gesamt", stats.games, `${stats.wins} Siege / ${stats.draws} Remis`],
  ...
  function renderDashboardOpponents() {
    ...
    els.opponentQuickTable.innerHTML = rows.map(({ opponent, record, lastMatch, avg }) => {
      return `
        <tr>
          <td>
  ...
  function renderAnalysis() {
    ...
    const bestSetup = getBestSetupSummary();
    const hardestOpponent = getOpponentPerformanceSummary("hardest");
  ...
  function buildAnalysisInsights(matches, stats) {
    ...
    const provenSetups = getSetupSummaries(5);
    ...
    const hardestOpponent = getOpponentPerformanceSummary("hardest");
    const bestOpponent = getOpponentPerformanceSummary("best");
    if (hardestOpponent) {
      insights.push({
        level: "warning",
  ...
  function resetAllData() {
    ...
    if (!second) return;
    app.data = { opponents: [], matches: [], setups: [] };
    app.editingOpponentId = null;
    app.editingMatchId = null;
    app.editingSetupId = null;
    app.selectedProfileId = null;
  ...
  function getBestSetupSummary() {
    const summaries = getSetupSummaries(1);
  ...
  function getSetupSummaries(minGames = 0) {
  ...
  function getOpponentPerformanceSummary(mode) {
    const rows = app.data.opponents.map((opponent) => ({
      opponent,
      record: calculateOpponentRecord(opponent.id)
  ...
  function opponentIncludesCategory(opponent, category) {
    ...
    if (category === "cup") return normalized === "cup" || normalized === "both";
    if (category === "liga") return normalized === "liga" || normalized === "both";
    if (category === "both") return normalized === "both";
    return true;
  }
